var Gmaps4Rails = {
		  ...
		  //check if the marker has not already been created
      	 ...
      	 var Lat = this.markers[i].latitude;
				 ...
				 // Marker sizes are expressed as a Size of X,Y
		 		 if (marker_picture === "")
					{ ThisMarker = new google.maps.Marker({position: myLatLng, map: this.map, title: marker_title,draggable:true});	}
					else 
				  {
  ...
  sidebar_element_handler: function(marker, eventType) {
	...
	add_Control: function () {
	  ...
	  var add_mark = document.createElement('DIV');
	  add_mark.style.backgroundColor = 'white';
	  add_mark.style.borderStyle = 'solid';
	  add_mark.style.borderWidth = '2px';
	  add_mark.style.cursor = 'pointer';
	  add_mark.style.textAlign = 'center';
	  add_mark.title = 'Click to add a marker to map';
	  controlDiv.appendChild(add_mark);

	  // Set CSS for the control interior
	  var add_mark_Text = document.createElement('DIV');
	  add_mark_Text.style.fontFamily = 'Arial,sans-serif';
	  add_mark_Text.style.fontSize = '12px';
	  add_mark_Text.style.paddingLeft = '4px';
	  add_mark_Text.style.paddingRight = '4px';
	  add_mark_Text.innerHTML = '<b>add marker</b>';
	  add_mark.appendChild(add_mark_Text);

	  // Setup the click event listeners: simply set the map to
	  // Chicago
	  google.maps.event.addDomListener(add_mark, 'click', function() {
		alert(Gmaps4Rails.map)
		  google.maps.event.addListener(Gmaps4Rails.map, 'click', function(event) {
	  ...
	  var add_line = document.createElement('DIV');
	  add_line.style.backgroundColor = 'white';
	  add_line.style.borderStyle = 'solid';
	  add_line.style.borderWidth = '2px';
	  add_line.style.cursor = 'pointer';
	  add_line.style.textAlign = 'center';
	  add_line.title = 'Click to add a line to map';
	  controlDiv.appendChild(add_line);
	 

	// Set CSS for the control interior
	  var add_line_Text = document.createElement('DIV');
	  add_line_Text.style.fontFamily = 'Arial,sans-serif';
	  add_line_Text.style.fontSize = '12px';
	  add_line_Text.style.paddingLeft = '4px';
	  add_line_Text.style.paddingRight = '4px';
	  add_line_Text.innerHTML = '<b>add line</b>';
	  add_line.appendChild(add_line_Text);
	 

	 google.maps.event.addDomListener(add_line, 'click', function() {
		alert(2)
	  });
